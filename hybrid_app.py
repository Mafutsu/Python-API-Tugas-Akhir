...
if st.button("Search"):
    ...
    for result in results:
        ...
        title = (
            f"UU No.{result['nomor']} "
            f"Tahun {result['tahun']}"
        )

        # tambah pasal kalau ada
        if result.get("pasal") not in [None, "", "-"]:

            title += f" Pasal {result['pasal']}"

        # tambah ayat kalau ada
        if result.get("ayat") not in [None, "", "-"]:

            title += f" Ayat {result['ayat']}"
